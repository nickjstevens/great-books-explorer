    ...
    <div className="page-shell">
      <header className="hero-card">
        ...
        <aside className="starter-card">
          <span className="eyebrow">Current starting point</span>
          <h2>{starterBook.title}</h2>
          <p>{starterBook.summary}</p>
          <button
            className="inline-link"
            onClick={() => {
              setSelectedBookId(starterBook.id)
              setDetailOpen(true)
            }}
            type="button"
          >
      ...
      <section className="territory-entry-grid">
      ...
      <section className="control-band">
          ...
          <label htmlFor="catalog-search">Search books or authors</label>
          <input
            id="catalog-search"
            onChange={(event) => {
              ...
              if (nextValue.trim()) {
                setViewMode('focus')
                setSelectedPathId('all')
              } else if (selectedTerritory === 'all' && selectedPathId === 'all') {
                setViewMode('overview')
              }
            }}
        ...
        <div className="control-group">
          ...
          <div className="chip-row">
            <button
              className={`chip ${effectiveTerritory === 'all' ? 'is-selected' : ''}`}
            ...
            {territories.map((territory) => (
              <button
                key={territory.id}
                className={`chip ${effectiveTerritory === territory.id ? 'is-selected' : ''}`}
        ...
        <div className="control-group small-group">
          ...
          <p className="control-note">Keep the first view tight and decision-ready.</p>
        </div>
      </section>

      <section className="active-path-card" id="active-path">
        <div>
          <span className="eyebrow">{isOverviewMode ? 'Overview mode' : 'Active path'}</span>
          <h2>{isOverviewMode ? 'Overview across the strongest visible anchors' : activePath?.name}</h2>
          <p>
            {isOverviewMode
              ? 'Scan the atlas by territory first, then drop into a path when a region of the canon starts pulling on you.'
        ...
        <div className="segmented-control">
          ...
          <button
            ...
            onClick={() => setOrderMode('conceptual')}
            type="button"
          >
            Conceptual
          </button>
      ...
      <section className="atlas-grid">
        <article className="panel">
            <div className="panel-heading">
              ...
              <h3>{isOverviewMode ? 'Read the atlas through its doorway books' : `Read this path in ${orderMode} order`}</h3>
              </div>
            </div>
        ...
        <article className="panel wide-panel">
          <div className="panel-heading">
            <div>
              ...
              <h3>
                {activePath
                  ? `${activePath.name} as a spine and clusters`
      ...
      <section className="panel territory-map-panel">
        <div className="panel-heading">
          <div>
        ...
        <div className="territory-map-grid">
          {territoryGroups.map(({ territory, books: territoryBooks }) => (
            ...
            >
              ...
              <div className="node-pile">
                {territoryBooks.slice(0, deferredSearch ? 12 : 8).map((book: Book) => (
                  <button
                    key={book.id}
                    className={`book-pill ${selectedBook.id === book.id ? 'is-active' : ''}`}
                    onClick={() => openBook(book.id)}
                    type="button"
                  >
                    {book.title}
        ...
        <div className="detail-backdrop" onClick={() => setDetailOpen(false)} role="presentation">
          ...
          >
            ...
            <div className="detail-body">
              ...
              <section>
                <span className="eyebrow">Why it sits on this path</span>
                <p>
                  {activePath
...
function buildGraphLayout(graphBooks: typeof books) {
  const width = 960
  const columns = Math.max(3, Math.ceil(graphBooks.length / 2))
  const columnWidth = width / columns
  const positions: Record<string, { x: number; y: number }> = {}

  graphBooks.forEach((book, index) => {
    const column = index % columns
    const row = Math.floor(index / columns)
    positions[book.id] = {
      x: columnWidth * column + columnWidth / 2,
      y: 120 + row * 190 + (column % 2 === 0 ? 0 : 24),
    }
  })

  const height = 220 + Math.max(0, Math.ceil(graphBooks.length / columns) - 1) * 190
...
function buildGraphBookIds({
  ...
  if (isOverviewMode) {
    return visibleBooks.slice(0, 12).map((book) => book.id)
  }
  ...
  if (activePath) {
    ...
    return [...new Set([...pathFirst, ...related, ...visibleBooks.slice(0, 12)].map((book) => book.id))].slice(0, 12)
  }
